...
const Dashboard = () => {
  ...
  // Real-time simulation for health metrics
  ...
  React.useEffect(() => {
    const interval = setInterval(() => {
      setHealthData(prev => {
        // Simulate random changes for each metric
        const steps = {
          ...prev.steps,
          value: Math.max(0, prev.steps.value + Math.floor(Math.random() * 10 - 5))
        };
        const heartRate = {
          ...prev.heartRate,
          value: Math.max(55, Math.min(120, prev.heartRate.value + Math.floor(Math.random() * 3 - 1)))
        };
        const bloodPressure = {
          ...prev.bloodPressure,
          systolic: Math.max(110, Math.min(130, prev.bloodPressure.systolic + Math.floor(Math.random() * 2 - 1))),
          diastolic: Math.max(70, Math.min(90, prev.bloodPressure.diastolic + Math.floor(Math.random() * 2 - 1)))
        };
        const oxygenSat = {
          ...prev.oxygenSat,
          value: Math.max(95, Math.min(100, prev.oxygenSat.value + Math.floor(Math.random() * 2 - 1)))
        };
        const temperature = {
          ...prev.temperature,
          value: Math.max(97.5, Math.min(99.5, prev.temperature.value + (Math.random() * 0.2 - 0.1)))
        };
        const weight = {
          ...prev.weight,
          value: Math.max(65, Math.min(75, prev.weight.value + (Math.random() * 0.05 - 0.02)))
        };
        const sleepHours = { ...prev.sleepHours };
        const waterIntake = {
          ...prev.waterIntake,
          value: Math.max(0, Math.min(12, prev.waterIntake.value + (Math.random() > 0.95 ? 1 : 0)))
        };
        const calories = {
          ...prev.calories,
          burned: Math.max(0, prev.calories.burned + Math.floor(Math.random() * 5))
        };
        // Health score calculation
        const healthScore = Math.max(60, Math.min(100, Math.round(
          0.2 * (steps.value / steps.target * 100) +
          0.1 * (100 - Math.abs(heartRate.value - 72)) +
          0.1 * (100 - Math.abs(bloodPressure.systolic - 120)) +
          0.1 * (100 - Math.abs(bloodPressure.diastolic - 80)) +
          0.1 * (oxygenSat.value) +
          0.1 * (100 - Math.abs(temperature.value - 98.6) * 10) +
          0.1 * (100 - Math.abs(weight.value - 68.5) * 2) +
          0.1 * (sleepHours.value / sleepHours.target * 100)
        )));
        return {
          ...prev,
          steps,
    ...
    <Container maxWidth="lg" sx={{ mt: 4, mb: 4 }}>
      ...
      <Box sx={{ display: 'grid', gap: 3, gridTemplateColumns: 'repeat(auto-fit, minmax(350px, 1fr))' }}>
        ...
        >
          ...
          <Box sx={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 2, mt: 2 }}>
            <Box>
              <Typography variant="h4" color="secondary.main">{(healthData.steps?.value ?? 0).toLocaleString()}</Typography>
              <Typography variant="body2" color="text.secondary">Steps Today</Typography>
            </Box>
            <Box>
              <Typography variant="h4" color="error.main">{healthData.heartRate?.value ?? '--'}</Typography>
              <Typography variant="body2" color="text.secondary">Heart Rate (bpm)</Typography>
            </Box>
            <Box>
              <Typography variant="h4" color="info.main">{healthData.weight?.value ?? '--'} {healthData.weight?.unit ?? 'kg'}</Typography>
              <Typography variant="body2" color="text.secondary">Current Weight</Typography>
            </Box>
            <Box>
              <Typography variant="h4" color="success.main">{healthData.sleepHours?.value ?? '--'}h</Typography>
      ...
      <Box sx={{ mt: 4, display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 2 }}>
        <Paper sx={{ p: 2, textAlign: 'center' }}>
          <Typography variant="h5" color="info.main">{healthData.waterIntake?.value ?? '--'}/{healthData.waterIntake?.target ?? 8}</Typography>
          <Typography variant="body2" color="text.secondary">Glasses of Water</Typography>
        </Paper>
        <Paper sx={{ p: 2, textAlign: 'center' }}>
          <Typography variant="h5" color="warning.main">{healthData.calories?.consumed ?? '--'}</Typography>
